...
function hasOverlap(start, end){
  return getBookings().some(b => overlaps(start, end, b.checkin, b.checkout));
}
...
setupPreview('icFront','icFrontPreview');
setupPreview('icBack','icBackPreview');
function render(){
  tbody.innerHTML = '';
...
form.addEventListener('submit', async e => {
  ...
  const icFrontFile = document.getElementById('icFront').files[0];
  const icBackFile = document.getElementById('icBack').files[0];
  if(!icFrontFile || !icBackFile){ message.className='error'; message.textContent='Sila masukkan gambar IC depan dan belakang.'; return; }
  ...
  const payload = {
    ...
    icFrontImage: await fileToBase64(icFrontFile),
    icBackImage: await fileToBase64(icBackFile),
    malam: nights,
  ...
  render(); form.reset();
  document.getElementById('icFrontPreview').style.display = 'none';
  document.getElementById('icBackPreview').style.display = 'none';
